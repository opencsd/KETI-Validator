...
ORDER  BY cntrycode;");
          parsed_query.Set_Query_Type_As_PushdownQuery();
        } else if(parsed_query.Get_Ori_Query() == "TPC-H_23"){ //custom query jhk
            parsed_query.Set_Parsed_Query("SELECT c_custkey, c_name, n_name\n\
            FROM  customer, nation\n\
            WHERE  c_nationkey = n_nationkey AND c_custkey < 10;");
          parsed_query.Set_Query_Type_As_PushdownQuery();
        }else if(parsed_query.Get_Ori_Query() == "TEST_01"){
